//functional requirment instance generated from row 83
Instance: CHE.FXREQ.009.C
InstanceOf: SGRequirements
Usage: #definition
* title = "Receive information messages in a way that ensures patient confidentiality"
* status = $pubStatus#active
* name = "Receive information messages in a way that ensures patient confidentiality"
* publisher = "WHO"
* experimental = true
* actor[+] = Canonical(healthworkerandneonateinfantchildcaregiver)
* extension[userstory].extension[capability].valueString = "Receive information messages in a way that ensures patient confidentiality"
* extension[userstory].extension[benefit].valueString = "I can manage my client's health in my daily life"
* description = """
*Business process* (C) Treat the child:
As a <a href="ActorDefinition-healthworkerandneonateinfantchildcaregiver.html">Health worker and neonate/infant/child (caregiver)</a>, I want to:
>Receive information messages in a way that ensures patient confidentiality

so that

>I can manage my client's health in my daily life
"""



